...
(function () {
  ...
  function buildTicker() {
    const v = VAULTS.find(x => x.id === selectedId);
    const t = window.TREASURY, price = window.TOKEN_PRICE, sc = window.SCAN;
    const items = [];
    items.push(price > 0
      ? `<span><b>$VAULT</b>$${price < 0.01 ? price.toFixed(6) : price.toFixed(4)} · Robinhood Chain</span>`
      : '<span><b>$VAULT</b>Robinhood Chain</span>');
    items.push(`<span><b>TREASURY</b>100% of fees · ${t > 0 ? '$' + (t >= 1000 ? Math.round(t).toLocaleString('en-US') : t.toFixed(2)) : 'reading…'} in the vault</span>`);
    if (v) items.push(`<span><b>ACTIVE VAULT</b>${v.name} · APR <span class="${v.apr >= 0 ? 'up' : 'down'}">${fmtPct(v.apr)}</span></span>`);
    if (sc) items.push(`<span><b>HOLDERS</b>${sc.verified.toLocaleString()} verified · ${sc.scanned.toLocaleString()} scanned</span>`);
    if (window.SUPPLY) items.push(`<span><b>SUPPLY</b>${Math.round(window.SUPPLY).toLocaleString('en-US')} $VAULT</span>`);
    items.push('<span><b>EPOCH #01</b>first snapshot Sep 22 · epochs every 7 days · no payouts yet</span>');
  ...
  // a treasury far smaller than the vault reads better as "<0.001%" than as "0.000%"
  const tvlShare = tvl => {
    const p = tvl > 0 ? (window.TREASURY || 0) / tvl * 100 : 0;
    return p > 0 && p < 0.001 ? '<0.001%' : p.toFixed(3) + '%';
  };
  ...
  function renderDetail() {
    ...
    const weekly = apr / 52;
    const why = v.live ? [
      'Highest 30D APR among the vaults that pass the policy',
      'TVL is deep enough to take the treasury without moving the vault',
      'Re-checked at every weekly snapshot, rotates out as soon as it stops leading',
    ] : {
      1: ['Protocol-run market making, deep liquidity', 'Lowest drawdown profile on Hyperliquid', 'Best fit for capital preservation'],
      2: ['Consistent 30d and 90d returns', 'Delta-neutral exposure, moderate drawdown', 'TVL large enough to absorb treasury deposits'],
      3: ['Directional strategy, higher variance', 'Returns depend on the trend regime', 'Sized for a partial allocation only'],
      4: ['High APR with high leverage', 'Short track record, deep drawdowns observed', 'Needs a full epoch of monitoring before it can lead'],
      5: ['Aggressive leveraged strategy, high variance', 'Small TVL, likely capacity limits', 'Monitored daily while it holds the treasury'],
    }[v.risk];
    $('#vaultDetail').innerHTML = `
      <div class="panel vd">
        <div class="vd__head">
          <div><span class="k">Active vault · auto-selected</span><h3>${v.name}</h3><span class="muted mono">${v.leader}${v.live ? '' : ' · age ' + v.age + 'd'}</span></div>
        ...
        <div class="vd__stats">
          ...
          <div><span class="k">Yield per epoch</span><span class="v mono">${fmtPct(weekly)}</span></div>
          <div><span class="k">TVL</span><span class="v mono">${fmtUSDc(v.tvl)}</span></div>
          <div><span class="k">Treasury share of TVL</span><span class="v mono">${tvlShare(v.tvl)}</span></div>
        </div>
      </div>
      <div class="panel vd__why">
        <h4>Why it was picked${v.live ? '' : ' · risk ' + v.risk + '/5'}</h4>
        <ul>${why.map(w => `<li>${w}</li>`).join('')}</ul>
        <p class="fine" style="margin-top:16px">Policy: TVL ≥ $1M, age ≥ 120 days, highest 30D APR wins. The leading vault at each weekly snapshot receives the treasury automatically, with no vote and no manual step.</p>
      </div>`;
    drawSpark($('#vdChart'), v.spark, apr >= 0 ? css('--lime') : css('--red'));
  }

  /* ---------- live vault (Hyperliquid) ----------
     chain.js calls this with the real name / APR / TVL of the vault in CONFIG.VAULT.
     It replaces the snapshot row so the table, the terminal and the calculator agree. */
  window.setVault = function (info) {
    const v = VAULTS.find(x => x.id === selectedId);
    if (!v || !info) return;
    if (info.name) v.name = info.name;
    if (Number.isFinite(info.apr)) v.apr = info.apr;
    if (Number.isFinite(info.tvl)) v.tvl = info.tvl;
    if (info.leader) v.leader = info.leader.slice(0, 6) + '...' + info.leader.slice(-4);
    if (info.age) v.age = info.age;
    v.live = true;
    // the snapshot row for the same vault would otherwise appear twice
    for (let i = VAULTS.length - 1; i >= 0; i--) {
      if (VAULTS[i].id !== v.id && VAULTS[i].name === v.name) VAULTS.splice(i, 1);
    }
    $('#tVault').textContent = v.name;
    $('#tApr').textContent = fmtPct(v.apr);
    $('#tApr').className = 'v mono ' + (v.apr >= 0 ? 'up' : 'down');
    renderTable(); renderDetail(); calc(); buildTicker();
  };
  ...
  function calc() {
    ...
    const ELIGIBLE = window.ELIGIBLE || 0.42, AVG_MULT = window.AVG_MULT || 2;
  ...
  const EPOCHS = [
    ...
    ['#01', 'Sep 22 2026', 'next', -1, 'First snapshot Sep 22 00:00 UTC · 7-day epoch'],
  ...
  function drawCluster(verified = 1204, excluded = 1666) {
    ...
    out += `<text x="512" y="18" font-family="${css('--mono')}" font-size="10" fill="${red}" text-anchor="end">SOLD OUT · ${excluded.toLocaleString()} wallets · excluded</text>`;
    out += `<text x="8" y="296" font-family="${css('--mono')}" font-size="10" fill="${lime}">STILL HOLDING · ${verified.toLocaleString()} wallets</text>`;
    svg.innerHTML = out;
  }
